  ...
  if (s.strings[1] = 'DISCONNECT') then //  if (s.strings[1] = 'DISCONNECT') and (s.strings[2] = ConnectionID) then CallIn.Hide;
    begin
      ...
      if assigned(CallIn) then 
        if Count > 0 then Callin.ShowCall(0);  //den ersten Anruf zeigen, wenn der letzte wegf‰llt
...
begin
 ...
 for i:=0 to SL.Count-1 do
  ...
  begin
    ...
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&auml;','‰');
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&ouml;','ˆ');
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&uuml;','¸');
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&Auml;','ƒ');
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&Ouml;','÷');
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&Uuml;','‹');
    Phonebook[len].Name   := AnsireplaceStr(Phonebook[len].Name, '&szlig;','ﬂ');
  end;
 r.Free;
 SL.Free;
end;
